...
fn addVariant(
    ...
    const exe = b.addExecutable(.{
        .name = b.fmt("arithmetic_{s}", .{ld_mode}),
    ...
    exe.addAssemblyFile(b.path("src/add_u32.S"));
    exe.addAssemblyFile(b.path("src/addi_u32.S"));
    exe.addAssemblyFile(b.path("src/sub_u32.S"));
    ...
    const install_bin = b.addInstallBinFile(
        bin.getOutput(),
        b.fmt("arithmetic_{s}.bin", .{ld_mode}),
    );
